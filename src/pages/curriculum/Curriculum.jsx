  ...
  const deleteHandler = (plan) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosInstance
          .delete(`/curriculum-file/${plan.id}?userId=${plan.UserId}`)
          .then((data) => {
            console.log(data);
            ...
            <label>
              ...
              <div class="text flex gap-3 items-center bg-indigo-600 text-white border border-gray-300 rounded font-semibold cursor-pointer p-1 px-3 hover:bg-indigo-500">
                {file?.name ? (
                  ...
                  <span>select curriculum to upload</span>
                )}
              </div>
            </label>
        ...
        >
          ...
          <AccordionDetails>
            {isFetching ? (
              ...
              <>
                {result?.length == 0 && <p>No Curriculum uploaded yet.</p>}
                {result?.map((plan) => (
